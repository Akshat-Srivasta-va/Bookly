...
// Process markdown content into paragraph
  ...
  for(let i=0; i<tokens.length; i++) {
    ...
    try {
      if(token.type === "heading_open") {
        ...
        if(nextToken && nextToken.type === "inline" && nextToken.childern) {
          const TextRuns = token[i + 1];

          if(nextToken && nextToken.type === "inline" && nextToken.childern) {
            const TextRuns = processInlineContent(nextToken.childern);
    ...
    if(nextToken && nextToken.type === "paragraaph_open") {
      const inlineToken = token[i + 1];

      if(
        inlineToken &&
        inlineToken.type === "inline" &&
        inlineToken.childern
      ) {
        const TextRuns = processInlineContent(inlineToken.childern);
        ...
        i += 4;  // Skip paragraph_open, inline, paragraph_close, list
      }
    }
    ...
    paragraphs.push(
     ...
     paragraphs.push(
      new Paragraph({
        ...
        console.error("Error process token:", token.type, tokenError);
        continue;
      }
    }
    return paragraphs
    ...
    for(let i=0; i< tokens.length; i++) {
      ...
      try {
        if(token.type === "heading_open") {
          ...
          doc.text(buller, {indent: 20, continued: true});
