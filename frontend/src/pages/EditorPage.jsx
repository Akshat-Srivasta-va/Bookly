...
const EditorPage = () => {
  ...
  const handleCoverImageUpload = async (e) => {
    ...
    try {
      const response = await axiosInstance.put(
        `${API_PATHS.BOOKS.GET_BOOKS}/${bookId}/cover`,
        formData,
        {
  ...
  try {
    ...
    setBook(updatedBook);
    toast.success(`Content for "${chapter.title}" generaated`)
      ...
      <div className="flex bg-slate-50 font-sans relative min-h-screen">
        ...
        {isSidebarOpen && (
          ...
          >
            ...
            ></div>
            <div className="relative flex-1 flex flex-col max-w-xs bg-white">
              <div className="absolute top-0 right-0 -mr-12 pt-12">
                <button
                  type="button"
          ...
          <main className="flex-1 h-full flex flex-col">
            <header className="sticky top-0 z-10 bg-white/80 backdrop-blur-sm border-b border-slate-200 p-3 flex justify-between items-center">
              ...
              <div className="flex items-center gap-2 sm:gap-4">
                ...
                >
                  <DropdownItems onClick={handleExportPDF}>
                    <FileText className="w-4 h-4 mr-2 text-slate-500"/>
                    Export as PDF
                  </DropdownItems>
                  <DropdownItems onClick={handleExportDoc}>
                    <FileText className="w-4 h-4 mr-2 text-slate-500 "/>
                    Export as Document
                  </DropdownItems>
                </Dropdown>
